...
export default function Button({ variant = 'primary', fullWidth = false, className = '', children, ...props }: ButtonProps) {
  const base = 'font-semibold transition-all cursor-pointer disabled:opacity-40'
  const width = fullWidth ? 'w-full' : ''

  const variants = {
    primary: 'bg-accent hover:bg-accent-hover text-black text-[0.83rem] font-bold px-6 py-3 border-0 rounded-lg',
    ghost:   'bg-transparent border border-[#222] text-secondary text-[0.83rem] px-6 py-3 hover:border-[#444] hover:text-primary rounded-lg',
